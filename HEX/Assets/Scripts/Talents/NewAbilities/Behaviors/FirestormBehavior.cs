...
public class FirestormBehavior : BaseAbilityBehavior
{
    private int _damage;
    ...
    public FirestormBehavior(int damagePerTick, float radius, float duration)
    {
        ...
        if (PhotonView.isMine)
        {
            ...
            if (Time.time - _lastTick > 0.5f)
            {
                ...
                foreach (PhotonView other in _playersToDamage)
                {
                    ...
                    Debug.Log("Giving firestorm dmg");
